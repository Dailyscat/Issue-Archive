...
HttpClient 커넥션 풀에 connectionRequestTimeout 짧게 설정해서 풀 앞 대기 시간 상한 두기

도메인별로 socketTimeout 단축, 중요/비중요 API 구분해서 타임아웃/디그레이드 전략 분리

읽기 위주의 MSA 호출에 캐시 확대해서 피크 시 외부 호출 수 자체 줄이기
...
커넥션 풀이 꽉 찼을 때, 커넥션을 빌리려고 무한 대기하는 요청을 빨리 잘라내기
...
<br/> <br/> 2) socketTimeout 재조정 (도메인별로)
<br/> <br/>
...
<br/> <br/> 4) 부가 기능에 fail-fast + 디그레이드 적용
<br/> <br/>
...
<br/> <br/> 5) (중기) 구조 개선: 비동기/서킷브레이커
<br/> <br/>

WebClient를 진짜 리액티브로 쓰거나, 최소한 병렬 호출로 전환
